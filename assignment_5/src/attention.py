...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        XA = self.w1(x).view(B,T,self.n_head,C//self.n_head).transpose(1,2) #(B, nh, T, hs)
        XV = self.value(x).view(B,T,self.n_head,C//self.n_head).transpose(1,2) #(B, nh, T, hs)

        #print(f'shape of XA {XA.shape}')
        #print(f'shape of XV {XA.shape}')

        XA = F.relu(XA)
        XAB = torch.matmul(XA,self.w2[:,:T])+self.b2[:T] # (B,nh,T,hs) x (hs,T) -> (B,nh,T,T)
        #print(f'shape of B: {self.w2.shape}')
        
        XAB = XAB.masked_fill(self.mask[:,:,:T,:T] == 0, -1e10) # todo: just use float('-inf') instead?

        
        XAB = F.softmax(XAB,dim=-1)

        XAB = self.attn_drop(XAB)

        #print(f'shape of XAB {XAB.shape}')

        Y = torch.matmul(XAB,XV) # (B,nh,T,T) x (B,nh,T,hs) -> (B,nh,T,hs)
