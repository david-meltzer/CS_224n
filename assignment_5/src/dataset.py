...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        max_size = int(self.block_size*7/8)

        rand_length = random.randint(4,max_size)
        #rand_length = max_size
        #start_pos = random.randint(0,len_doc-rand_length)
        #
        #trunc_doc = document[start_pos:start_pos+rand_length]

        trunc_doc = document[:rand_length]

        mask_size = random.randint(int(len_doc/4-len_doc/12),int(len_doc/4+len_doc/12))
        prefix_size = int((len(trunc_doc)-mask_size)/2)

        prefix = trunc_doc[:prefix_size]
        masked_content = trunc_doc[prefix_size:prefix_size+mask_size]
        suffix = trunc_doc = trunc_doc[prefix_size+mask_size:]

        new_doc = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        new_doc = new_doc + self.PAD_CHAR*(self.block_size-len(new_doc))
