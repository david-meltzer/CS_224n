...
if args.function == 'pretrain':
    ...
    tconf_pre = trainer.TrainerConfig(max_epochs=650,batch_size=128,learning_rate=6e-3,lr_decay=True,warmup_tokens=512*20,final_tokens=200*len(pretrain_dataset)*block_size,num_workers=4)
    pretrain_text = open(args.pretrain_corpus_path).read()
    train_model = trainer.Trainer(model,pretrain_dataset,None,tconf_pre)
    train_model.train()
    ...
    fine_tune_text = open(args.finetune_corpus_path).read()
    data_train = dataset.NameDataset(pretrain_dataset,fine_tune_text)
    
    train_model = trainer.Trainer(model,data_train,None,tconf)
    train_model.train()
    torch.save(model.state_dict(),args.writing_params_path)
    ...
    with open(args.outputs_path, 'w') as fout:
        ...
        for line in tqdm(open(args.eval_corpus_path)):
           ...
           #print(f'device of pred is {pred.get_device()}')
