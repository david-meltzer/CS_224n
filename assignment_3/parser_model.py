...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(self.embed_size*n_features,self.hidden_size))
    ...
    def forward(self, w):
        ...
        hidden_state = F.relu(torch.matmul(embeds,self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        hidden_state = self.dropout(hidden_state)
        logits = torch.matmul(hidden_state,self.hidden_to_logits_weight)+self.hidden_to_logits_bias 
