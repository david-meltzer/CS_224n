...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        sent_temp =self.sentence.copy()
        self.stack=['ROOT']
        self.buffer=sent_temp
        self.dependencies = []
    ...
    def parse_step(self, transition):
        ...
        if transition=="S":
            ...
            removed_element = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1],removed_element))
        elif transition == 'RA':
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        mini_batch = [mini_batch[i].parse_step(transitions[i]) for i in range(len(mini_batch))]
        
        unfinished_parses = [up for up in unfinished_parses if not(len(up.stack)==1 and len(up.buffer)==0 )]
